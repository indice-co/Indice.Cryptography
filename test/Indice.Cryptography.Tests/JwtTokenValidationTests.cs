...
public class JwtTokenValidationTests
{
    ...
    public void JwtTokenValidationTest() {
        ...
        var validationParameters = new TokenValidationParameters {
            IssuerSigningKey = new X509SecurityKey(X509CertificateLoader.LoadCertificate(Convert.FromBase64String(TEST_X509_PublicKey_2048))),
...
public class JwtTokenValidation
{
    public async Task<Dictionary<string, X509Certificate2>> FetchGoogleCertificates() {
        using (var http = new HttpClient()) {
            ...
            return dictionary.ToDictionary(x => x.Key, x => X509CertificateLoader.LoadCertificate(Encoding.UTF8.GetBytes(x.Value)));
        }
    }

    private readonly string CLIENT_ID = "xxxx.apps.googleusercontent.com";
